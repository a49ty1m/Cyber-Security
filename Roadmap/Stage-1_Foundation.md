...
- [Module 02: Linux Administration](#module-02-linux-administration)
  ...
  - [Topic 4: Log Analysis & Monitoring (Linux)](#stage-4-log-analysis-monitoring-linux)
...
- [Module 03: Windows Administration](#module-03-windows-administration)
  ...
  - [Topic 4: PowerShell Administration (Windows)](#stage-4-powershell-administration-windows)
  - [Topic 5: Active Directory Concepts (Prerequisite for Module 19)](#stage-5-active-directory-concepts-prerequisite-for-part-23)
...
- [ ] **Capture & Evidence:** Wireshark installed on host and Kali; verify promiscuous mode packet captures work on host-only interface.
- [ ] **Documentation Git Repo:** Initialize a private repository (`~/Cyber-Security_Notes`) to commit notes, scripts, and PCAPs after every lab session.
...
- [ ] **CPU Cache Hierarchy (L1/L2/L3):** Understand the three cache levels: **L1** (per-core, ~32–64 KB, fastest, split into instruction and data cache), **L2** (per-core, ~256 KB–1 MB, slightly slower), **L3** (shared across all cores, several MB, last stop before main RAM). Data is loaded into cache in **cache lines** (typically 64 bytes). Security relevance: **Spectre and Meltdown** exploit speculative execution + cache timing — an attacker measures how long a cache hit vs miss takes (`rdtsc`) to infer data that should be inaccessible. **Flush+Reload**, **Prime+Probe**, and **Evict+Time** are cache side-channel attack families. Understanding caches is required reading before any microarchitectural side-channel research.
...
- [ ] **TPM (Trusted Platform Module):** Understand the **TPM 2.0** chip as a dedicated hardware cryptographic co-processor soldered to the motherboard (or built into the CPU as fTPM). Core functions: **hardware-bound key storage** (keys never leave the chip in plaintext), **remote attestation** (prove to a verifier that a machine booted an expected software stack by signing a PCR quote), **sealing** (encrypt data to a specific system state — if the boot chain changes, the data cannot be decrypted). **PCRs (Platform Configuration Registers)** are hash chains of boot measurements — BIOS, bootloader, kernel, initrd each extend a PCR. **BitLocker** uses the TPM to seal the volume encryption key to PCR values — tamper the boot chain and BitLocker prompts for a recovery key. **TPM-based attacks:** PCIe bus sniffing on discrete TPMs to intercept the BitLocker key in transit (patched in modern systems with encrypted bus protocols). Know that **CVE-2023-1017/1018 (TPM2.0 reference implementation buffer overflows)** demonstrate that even the TPM itself can be attacked.

- [ ] **PCIe (Peripheral Component Interconnect Express):** Understand PCIe as the high-speed serial bus connecting the CPU to GPUs, NVMe SSDs, NICs, and add-in cards. Each device communicates over **lanes** (x1, x4, x8, x16). Security relevance: PCIe devices have **DMA capability** — they can directly read and write system RAM without CPU involvement. **Thunderbolt** (Intel) uses PCIe tunneling — plugging a malicious Thunderbolt device gives it DMA access to RAM before the OS loads, enabling attacks like **Evil Maid** (extract BitLocker keys from RAM) and **cold boot** variants. **PCIe-based DMA implants** (e.g., PCILeech / FPGA-based) are real red team hardware capable of reading/writing arbitrary RAM addresses from a plugged-in device. Mitigations: **IOMMU** (Intel VT-d / AMD-Vi) restricts which memory addresses each DMA-capable device can access — must be enabled in BIOS and OS. Know that IOMMU is disabled by default on many consumer systems.

- [ ] **USB Architecture & Attack Surface:** Understand the **USB host/device/hub** hierarchy and **USB descriptors** — when a USB device is plugged in, it presents a **device descriptor** (vendor ID, product ID), **configuration descriptors**, and **interface descriptors** declaring what class of device it is (HID, mass storage, CDC). The OS trusts these descriptors without authentication. **BadUSB:** Re-flash the firmware of a USB controller (e.g., Phison 2251-03) to present as a **HID keyboard** even if it looks like a USB flash drive — the OS treats all HID input as trusted. **Rubber Ducky / Hak5:** Commercial HID injection platforms that execute keystroke sequences at machine speed. **USBKill:** Charges capacitors via USB power pins and discharges into the host, destroying hardware. **Juice Jacking:** Charging via public USB ports where the data pins can exfiltrate data or deliver payloads. **USB drop attacks:** Leaving malicious drives in physical locations — curiosity-driven insertion rates are surprisingly high. Mitigations: **USBGuard** (Linux), **Windows USB Device Control**, and endpoint DLP policies that whitelist authorized USB device classes/IDs.
...
- [ ] **Embedded Controllers (EC) & BMC/IPMI:** **Embedded Controllers** are small microcontrollers inside laptops (typically an **ITE IT8xxx** or similar) that independently manage power sequencing, fan control, keyboard scanning, battery charging, and lid-switch detection — they run their own firmware and remain active even when the main OS is powered off. **Security relevance:** EC firmware persistence (e.g., **ThinkPwn** — a Lenovo UEFI/EC vulnerability) can survive OS reinstalls and BIOS reflashes. In servers, the equivalent is the **BMC (Baseboard Management Controller)** — a dedicated ARM SoC running **IPMI** or **Redfish** that provides out-of-band remote management (power cycle, serial console, virtual media) independently of the main CPU. **IPMI attack surface:** Default credentials (`ADMIN`/`ADMIN`), **IPMI 2.0 RAKP authentication hash disclosure** (the server sends a salted hash that can be cracked offline — Metasploit `auxiliary/scanner/ipmi/ipmi_dumphashes`), **cipher 0 authentication bypass**, and direct memory access via **IPMI KVM** without OS involvement. Compromised BMC = persistent access that survives OS reinstalls, disk replacements, and power cycling. Know these exist; full exploitation is covered in advanced hardware security content.
...
- [ ] **Essential Libraries & APIs:** Gain familiarity with:
  ...
  - [ ] Boost _(awareness)_
  - [ ] COM (Component Object Model) — understand COM as the Windows binary interface standard for inter-process communication and component reuse. Every COM object has a **CLSID** (Class ID, a GUID) registered in `HKCR\CLSID\`. Clients instantiate objects via `CoCreateInstance()`. COM objects run **in-process** (DLL loaded into caller), **out-of-process** (separate EXE via DCOM), or in a **surrogate process**. Security relevance for RE: COM is ubiquitous in Windows — shell extensions, Office automation, WMI providers, and security products all use COM. When reversing a Windows binary, COM call patterns (`QueryInterface`, `AddRef`, `Release`, vtable dispatch) are a key pattern to recognise. **COM hijacking** (a persistence/privilege escalation technique) works by placing a malicious DLL at a HKCU registry path that Windows searches before HKLM — no admin rights required; the malicious COM object loads whenever a privileged process instantiates the targeted CLSID. Detect via Procmon (`RegQueryValue` on CLSID paths returning `NAME NOT FOUND`) and Sysmon Event ID 7.
...
- [ ] **Windows Hello & Modern Authentication:** Understand **Windows Hello for Business (WHfB)** as Microsoft's passwordless authentication platform. WHfB replaces the user's password with a **public/private key pair** or **certificate** backed by the TPM — the private key never leaves the device. Authentication to Azure AD / on-prem AD uses a **PKINIT Kerberos flow** (certificate-based TGT request) or **cloud-kerberos trust**. **Windows Hello (consumer)** uses biometrics (fingerprint, IR face recognition) or a PIN — critically, the PIN is **local to the device** and unlocks the TPM-stored private key; it is NOT sent over the network. Security relevance: WHfB defeats password spraying, credential stuffing, and NTLM relay attacks. However, **Golden Ticket** and **certificate-based attacks (ADCS ESC14)** still apply in hybrid environments. Attacking WHfB requires compromising the endpoint's TPM or the issuing ADCS CA — not the user's password.

- [ ] **Windows Credential Providers & DPAPI:** Understand that the Windows **Logon UI (winlogon.exe)** is extensible via **Credential Provider DLLs** (`HKLM\SOFTWARE\Microsoft\Windows\CurrentVersion\Authentication\Credential Providers`). A Credential Provider is a COM object that collects authentication input (password, smart card PIN, biometric) and passes it to the **LSA (Local Security Authority)** via the **Authentication Package** (MSV1_0 for NTLM, Kerberos SSP). **Attack surface:** Malicious credential providers are a persistence and credential-harvesting technique — an attacker with admin access registers a rogue DLL that logs every interactive logon credential. Defenders detect this via registry monitoring of the Credential Providers key and Sysmon Event ID 7 (DLL load from winlogon.exe). **DPAPI (Data Protection API):** The Windows subsystem protecting secrets encrypted to a user or machine — browser saved passwords, WiFi PSKs, RDP credentials, and certificate private keys are all DPAPI-sealed. The DPAPI master key is derived from the user's password hash — extracting it from LSASS or offline (with the NTLM hash or domain backup key `HKLM\SECURITY\Policy\Secrets\DPAPI_SYSTEM`) is standard post-exploitation. Tools: `mimikatz dpapi::`, `pypykatz`, Impacket `dpapi.py`.
...
- [ ] **Cryptographic Randomness — CSPRNG vs PRNG:**
  ...
  - **IV/nonce generation:** Always generated with a CSPRNG. A predictable IV in CBC or a repeated nonce in GCM/CTR is a critical vulnerability (see Topic 5).
